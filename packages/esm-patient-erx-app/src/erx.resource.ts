...
export function useProviderDetails(providerUuid?: string) {
  ...
  const { data, isLoading, error } = useSWR<{ data: ProviderDetails }, Error>(providerUrl, openmrsFetch);
...
export function useErxData(patientUuid: string, activeVisitUuid?: string) {
  ...
  const sortedVisitNotes =
    visitNotesData?.data?.results?.sort(
  ...
  const recentVisitNotes = sortedVisitNotes.slice(0, 3);
